...
import { Route, RouterLink, RouterModule,ActivatedRoute } from '@angular/router';
...
export class FavouritesComponent implements OnInit{
  ...
  ngOnInit(): void {
  this.favouritesRecipeList = Object.values(history.state).filter(item => typeof item === 'object');
}
  onDelete(item:any){
    const isDelete = confirm('Are you sure you want to delete the recipe?');
    if (isDelete) {
      // finding the currentRecord
      const currentRecord = this.favouritesRecipeList.findIndex((m:any) => m.id === item.id);
